...
const LevelsMessage: React.FC = () => {
  const { data, isLoading } = useQuery(`level-message`, () => LevelsMessageService.find())
  const [cookies, setCookie] = useCookies(['message_progress'])
  ...
  return (
    <div>
      ...
      <div className='flex flex-col items-center w-3/4 pb-20 mx-auto text-sec'>
        ...
        <Link
          to={answersIds.length < LEVELS_MESSAGE_NUMBER && nextLevel ? `/level-message/${nextLevel}` : ''}
          onClick={answersIds.length < LEVELS_MESSAGE_NUMBER ? () => {} : resetProgress}
        ...
        >
          <FontAwesomeIcon size='4x' icon={answersIds.length < LEVELS_MESSAGE_NUMBER ? faBook : faArrowRotateBack} />
          <p className='ml-20 text-3xl font-semibold'>
            {`${answersIds.length === 0 ? 'Rozpocznij naukę' : answersIds.length < LEVELS_MESSAGE_NUMBER ? 'Kontynuuj naukę' : 'Zresetuj postęp kursu'}`}
          </p>
        </Link>
        <p className='my-5 text-2xl'>Aktualny postęp kursu: {Math.round((answersIds.length/LEVELS_MESSAGE_NUMBER)*100)}% ({answersIds.length} z {LEVELS_MESSAGE_NUMBER} zadań)</p>
        <div className='flex flex-row'>
          <Link to={'/levels-message-tutorial'} className='flex items-center justify-center p-3 text-2xl text-white transition duration-200 rounded-full w-80 bg-pri hover:shadow-inner hover:opacity-85'>
            Zobacz szkolenie
          </Link>
          {
            answersIds.length !== LEVELS_MESSAGE_NUMBER &&
